...
// Black tea leaf color palette: pure solid black and subtle deep matte blacks as requested
const GRAIN_COLORS = [
  '#000000',
  '#050505',
  '#0A0A0A',
...
export const TeaLeavesSimulation: React.FC<TeaLeavesSimulationProps> = ({
  ...
  const createGrain = (customX?: number, customY?: number): TeaGrain => {
    ...
    // Pick color: 100% black in colour as requested
  ...
  useEffect(() => {
    ...
    const animate = (now: number) => {
      ...
      for (let i = grainsRef.current.length - 1; i >= 0; i--) {
        ...
        const px = grain.x * logicalWidth;
        const py = grain.y * logicalHeight;
        const ry = (grain.length * 0.5) * logicalHeight;
  ...
  return (
    <div className="w-full flex flex-col items-center select-none">
      ...
      <div
        ...
        className="relative max-w-[420px] w-full aspect-[1024/1041] cursor-pointer select-none"
      >
